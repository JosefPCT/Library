...
class Library{
    ...
    static addBookToLibrary(id, title, author, noOfPages, hasRead){
...
class ScreenController{
    ...
    // Method that is called on immediately when you create a new screenController
    initListeners(){
        let addBookButton = document.querySelector("#add-book-button");
        let submitBookButton = document.getElementById("submit-book");

        addBookButton.addEventListener("click", this.showaddBookForm);
        submitBookButton.addEventListener("click", this.submitBookForm);
    }

    // A static method of screenController to show an add book form
    ...
    // Method to submit the form, prevents the default of sending data to server, to manipulate data in JavaScript
    ...
    submitBookForm(e){
        ...
        Library.addBookToLibrary(crypto.randomUUID(), title, author, pages, readStatus);
        console.log(this);
        ScreenController.showBooks();
    ...
    static showBookToPage(item){
        ...
        bookDeleteButton.addEventListener("click", ScreenController.deleteBook);

        toggleReadButton.setAttribute("class", "toggle-read");
        toggleReadButton.addEventListener("click", ScreenController.toggleReadStatus);
    ...
    static showBooks(){
        ...
        Library.library.forEach(ScreenController.showBookToPage);
    }
    ...
    static deleteBook(e){
        ...
        // console.log(library);
        Library.library = Library.library.filter((item) => !(item.id === objectId));
        ScreenController.showBooks();
    }    
...
let myinit = new ScreenController;
